...
public class ActRTCompaniesDAO {
    ...
    public List<ActRTCompanies> showAll(){
        return jdbcTemplate.query("SELECT * FROM Акты_ПП_контрагенты LEFT JOIN Оборудование " +
                "ON Акты_ПП_контрагенты.инвентарная_карточка = Оборудование.инвентарная_карточка " +
                "LEFT JOIN Контрагенты ON Акты_ПП_контрагенты.инн = Контрагенты.инн LEFT JOIN Номенклатура ON Оборудование.код_номенклатуры = Номенклатура.код_номенклатуры",
...
/*    public List<ActRTCompanies> showAllTransmissions(){
    ...
    public ActRTCompanies showOne(Integer id){
        return jdbcTemplate.query("SELECT * FROM Акты_ПП_контрагенты LEFT JOIN Оборудование\n" +
                "ON Акты_ПП_контрагенты.инвентарная_карточка = Оборудование.инвентарная_карточка\n" +
                "LEFT JOIN Контрагенты ON Акты_ПП_контрагенты.инн = Контрагенты.инн LEFT JOIN Номенклатура ON Оборудование.код_номенклатуры = Номенклатура.код_номенклатуры WHERE код_документа = ?",
    ...
    public void save(ActRTCompanies actRTCompanies){
        jdbcTemplate.update("INSERT INTO Акты_ПП_контрагенты(инвентарная_карточка, дата_передачи, инн, дата_приемки) VALUES(?, ?, ?, ?)",
    ...
    public void update(Integer id, ActRTCompanies updatedAct){
        jdbcTemplate.update("UPDATE Акты_ПП_контрагенты SET инвентарная_карточка = ?, " +
    ...
    public void delete(Integer id){
        jdbcTemplate.update("DELETE FROM Акты_ПП_контрагенты WHERE код_документа = ?", id);
    }

    public List<ActRTCompanies> showCurrentActs(){
        return jdbcTemplate.query("SELECT * FROM Акты_ПП_контрагенты\n" +
                "LEFT JOIN Оборудование ON Акты_ПП_контрагенты.инвентарная_карточка = Оборудование.инвентарная_карточка\n" +
                "LEFT JOIN Номенклатура ON Оборудование.код_номенклатуры = Номенклатура.код_номенклатуры\n" +
                "LEFT JOIN Контрагенты ON Акты_ПП_контрагенты.инн = Контрагенты.инн\n" +
